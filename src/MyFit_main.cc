...
int main(int argc, char *argv[])
{
    ...
    // Create instance of MyFit class
    MyFit* ppi0 = new MyFit;
    ...
    {
        cout << "ERROR: MyFit Init failed!" << endl;
        return 0;
    }
    ...
    if(file_in.length() > 0)
    {
        ...
        if(length >= 5)
        {
            ...
            if(!ppi0->StartFile(file_in.c_str(), file_out.c_str())) cout << "ERROR: MyFit failed on file " << file_in << "!" << endl;
            files_found++;
        }
    }
            ...
            if(length >= 5)
            {
                ...
                if(((strcmp(prefix.c_str(),pre_in.c_str()) == 0)) && (strcmp(suffix.c_str(),".root") == 0))
                {
                    ...
                    if(!ppi0->StartFile(file_in.c_str(), file_out.c_str()))
                        cout << "ERROR: MyFit failed on file " << file_in << "!" << endl;
