...
{
    test1 	= new GHistTaggerBinning("test1", 	"test1", 	1400, -700, 700, 48);
    test2 	= new GHistTaggerBinning("test2", 	"test2", 	1400, -700, 700, 48);
    test3 	= new GHistTaggerBinning("test3", 	"test3", 	1400, -700, 700, 48);
...
void	PPi0Example::ProcessEvent()
{
    if(eta->GetNParticles()>0)
    {
        ...
        {
            test2->Fill(eta->Particle(0).M(), tagger->GetTagged_ch(t), tagger->GetTagged_t(t));
            test3->Fill(eta->Particle(0).M(), *tagger, kTRUE);
        }
    }
}
